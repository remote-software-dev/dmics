...
export default function DistrictReportsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setError("Failed to load district reports.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const filteredMr = useMemo(() => {
    ...
    return result;
  }, [mrReports, provinceFilter, districtFilter]);
  ...
  const filteredOpv = useMemo(() => {
    ...
    if (districtFilter) result = result.filter((r) => r.district === districtFilter);
    return result;
  }, [opvReports, provinceFilter, districtFilter]);

  const mrAggregated = useMemo(() => {
    const map = new Map<string, { name: string; province: string; total: number; count: number }>();
    for (const r of filteredMr) {
      const key = r.district || "Unknown";
      const existing = map.get(key) || { name: key, province: r.province || "-", total: 0, count: 0 };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-slate-500 text-sm">Loading district reports...</div>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-slate-900">District Reports</h1>
            <p className="text-xs text-slate-500">Vaccination data aggregated by district</p>
          </div>
        </div>
      </div>
              ...
              >
                ...
                {districts
                  .filter((d) => {
                    ...
                    return prov ? d.province_id === prov.id : true;
                  })
                  .map((d) => (
