...
export default function PuskesmasReportsPage() {
  ...
  const [puskesmasFilter, setPuskesmasFilter] = useState<string>("");
  ...
  const selectedProvinceId = useMemo(
    () => provinces.find((p) => p.name === provinceFilter)?.id || "",
    [provinces, provinceFilter]
  );

  const selectedDistrictId = useMemo(
    () => districts.find((d) => d.name === districtFilter)?.id || "",
    [districts, districtFilter]
  );

  const filteredMr = useMemo(() => {
    let result = mrReports;
    if (provinceFilter) result = result.filter((r) => r.province === provinceFilter);
    if (districtFilter) result = result.filter((r) => r.district === districtFilter);
    if (puskesmasFilter) result = result.filter((r) => r.puskesmas === puskesmasFilter);
    return result;
  }, [mrReports, provinceFilter, districtFilter, puskesmasFilter]);

  const filteredOpv = useMemo(() => {
    let result = opvReports;
    if (provinceFilter) result = result.filter((r) => r.province === provinceFilter);
    if (districtFilter) result = result.filter((r) => r.district === districtFilter);
    if (puskesmasFilter) result = result.filter((r) => r.puskesmas === puskesmasFilter);
    return result;
  }, [opvReports, provinceFilter, districtFilter, puskesmasFilter]);

  const mrAggregated = useMemo(() => {
    const map = new Map<string, { name: string; district: string; province: string; total: number; count: number }>();
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-slate-500 text-sm">Loading puskesmas reports...</div>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-slate-900">Puskesmas Reports</h1>
            <p className="text-xs text-slate-500">Vaccination data aggregated by puskesmas</p>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {districts
                  .filter((d) => !selectedProvinceId || d.province_id === selectedProvinceId)
                  .map((d) => (
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {puskesmasList
                  .filter((p) => {
                    if (selectedDistrictId) return p.district_id === selectedDistrictId;
                    if (selectedProvinceId) return p.province_id === selectedProvinceId;
                    return true;
                  })
                  .map((p) => (
                    <option key={p.id} value={p.name}>
                  ...
                  <td colSpan={6} className="px-5 py-12 text-center">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-xs text-slate-400 mt-1">
                        No {tab.toUpperCase()} reports available for the selected filters
                      </p>
                    </div>
                  </td>
                </tr>
