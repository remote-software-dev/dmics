...
export default function EditDistrictPage() {
    ...
    Promise.all([
      ...
      .then(([distData, provData]) => {
        setForm({
          code: distData.code || "",
          name: distData.name || "",
          province_id: distData.province_id?.toString() || "",
          latitude: distData.latitude?.toString() || "",
          longitude: distData.longitude?.toString() || "",
          is_active: distData.is_active ?? true,
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.detail || "Failed to update district");
      }
      router.push("/geographic/districts");
    } catch (e: any) {
      setError(e.message || "Failed to update district");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-slate-500 text-sm">Loading district...</div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900">Edit District</h1>
        <p className="text-slate-500 text-sm mt-1">Update district information</p>
      </div>
