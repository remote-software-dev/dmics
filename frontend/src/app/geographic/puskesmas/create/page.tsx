...
export default function CreatePuskesmasPage() {
  const { token } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.detail || "Failed to create puskesmas");
      }
      router.push("/geographic/puskesmas");
    } catch (e: any) {
      setError(e.message || "Failed to create puskesmas");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900">Create Puskesmas</h1>
        <p className="text-slate-500 text-sm mt-1">Add a new puskesmas</p>
      </div>
